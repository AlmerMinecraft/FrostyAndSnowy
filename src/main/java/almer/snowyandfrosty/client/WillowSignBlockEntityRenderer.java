...
public class WillowSignBlockEntityRenderer implements BlockEntityRenderer<SignBlockEntity> {
    ...
    private final Map<WoodType, WillowSignBlockEntityRenderer.SignModel> typeToModel;
    private final TextRenderer textRenderer;

    public WillowSignBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
        this.typeToModel = (Map)WoodType.stream().collect(ImmutableMap.toImmutableMap((signType) -> {
            return signType;
        }, (signType) -> {
            return new WillowSignBlockEntityRenderer.SignModel(ctx.getLayerModelPart(EntityModelLayers.createSign(signType)));
        }));
    ...
    public void render(SignBlockEntity signBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        ...
        WoodType woodType = ModWoodType.WILLOW;
        WillowSignBlockEntityRenderer.SignModel signModel = (WillowSignBlockEntityRenderer.SignModel)this.typeToModel.get(woodType);
    ...
    void renderSignModel(MatrixStack matrices, int light, int overlay, Model model, VertexConsumer vertexConsumers) {
        WillowSignBlockEntityRenderer.SignModel signModel = (WillowSignBlockEntityRenderer.SignModel)model;
    ...
    public static WillowSignBlockEntityRenderer.SignModel createSignModel(ModEntityModelLoader entityModelLoader) {
        return new WillowSignBlockEntityRenderer.SignModel(entityModelLoader.getModelPart(EntityModelLayers.createSign(ModWoodType.WILLOW)));
    }
