...
public class CalibratedHopperBlock extends BlockWithEntity {
    public static final MapCodec<CalibratedHopperBlock> CODEC = CalibratedHopperBlock.createCodec(CalibratedHopperBlock::new);
    ...
    public MapCodec<CalibratedHopperBlock> getCodec() {
        return CODEC;
    }

    public CalibratedHopperBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new CalibratedHopperBlockEntity(pos, state);
    }

    @Override
    @Nullable
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : CalibratedHopperBlock.validateTicker(type, ModBlockEntityType.CALIBRATED_HOPPER, CalibratedHopperBlockEntity::serverTick);
    }

    @Override
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        BlockEntity blockEntity;
        if (itemStack.hasCustomName() && (blockEntity = world.getBlockEntity(pos)) instanceof CalibratedHopperBlockEntity) {
            ((CalibratedHopperBlockEntity)blockEntity).setCustomName(itemStack.getName());
        }
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (blockEntity instanceof CalibratedHopperBlockEntity) {
            player.openHandledScreen((CalibratedHopperBlockEntity)blockEntity);
    ...
    public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof CalibratedHopperBlockEntity) {
            CalibratedHopperBlockEntity.onEntityCollided(world, pos, state, entity, (CalibratedHopperBlockEntity)blockEntity);
        }
    }
